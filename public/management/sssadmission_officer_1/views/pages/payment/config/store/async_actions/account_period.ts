...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    // );
    const response = await axios.get(`${end_point}/account-logs/periods`);

    dispatch(storeSlice.actions.set_is_loading(false));
    // dispatch(storeSlice.actions.set_item(response.data.data));
    dispatch(storeSlice.actions.set_period(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const periods = createAsyncThunk<ReturnType, PayloadType, ThunkArgument>(
    `${store_prefix}/periods`,
    fetch_api,
);
