...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(`${end_point}/user-teachers/teacher-all`);

    dispatch(storeSlice.actions.set_is_loading(false));
    dispatch(storeSlice.actions.set_teachers(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const all_teacher = createAsyncThunk<
    ReturnType,
    PayloadType,
    ThunkArgument
>(`${store_prefix}/all-teacher`, fetch_api);
