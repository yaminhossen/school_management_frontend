...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(`${end_point}/user-staffs/staff-all`);

    dispatch(storeSlice.actions.set_is_loading(false));
    dispatch(storeSlice.actions.set_staffs(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const all_staff = createAsyncThunk<
    ReturnType,
    PayloadType,
    ThunkArgument
>(`${store_prefix}/all-staff`, fetch_api);
