...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(`${end_point}/${api_prefix}/building`);

    dispatch(storeSlice.actions.set_is_loading(false));
    dispatch(storeSlice.actions.set_building(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const building = createAsyncThunk<
    ReturnType,
    PayloadType,
    ThunkArgument
>(`${store_prefix}/building`, fetch_api);
