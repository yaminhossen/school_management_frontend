...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(
        `${end_point}/${api_prefix}/class-wise-subject/${param.id}`,
    );

    dispatch(storeSlice.actions.set_is_loading(false));
    dispatch(storeSlice.actions.set_subjects(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const subjects = createAsyncThunk<
    ReturnType,
    PayloadType,
    ThunkArgument
>(`${store_prefix}/subjects`, fetch_api);
