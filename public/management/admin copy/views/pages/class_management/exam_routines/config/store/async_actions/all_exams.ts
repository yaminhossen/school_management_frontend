...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(`${end_point}/exams/all-exam`);

    dispatch(storeSlice.actions.set_is_loading(false));
    dispatch(storeSlice.actions.set_exams(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const exams = createAsyncThunk<ReturnType, PayloadType, ThunkArgument>(
    `${store_prefix}/exams`,
    fetch_api,
);
