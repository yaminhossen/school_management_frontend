...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(`${end_point}/${api_prefix}/rooms`);

    dispatch(storeSlice.actions.set_is_loading(false));
    dispatch(storeSlice.actions.set_rooms(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const rooms = createAsyncThunk<ReturnType, PayloadType, ThunkArgument>(
    `${store_prefix}/rooms`,
    fetch_api,
);
