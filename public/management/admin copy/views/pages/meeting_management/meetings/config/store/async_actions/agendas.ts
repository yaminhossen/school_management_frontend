...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(
        `${end_point}/${api_prefix}/meeting-agendas/${param.id}`,
    );

    dispatch(storeSlice.actions.set_is_loading(false));
    dispatch(storeSlice.actions.set_agendas(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const agendas = createAsyncThunk<ReturnType, PayloadType, ThunkArgument>(
    `${store_prefix}/agendas`,
    fetch_api,
);
