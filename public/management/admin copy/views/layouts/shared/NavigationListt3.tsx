...
const NavigationList: React.FC<Props> = () => {
    ...
    const handleLogout = async (e: React.MouseEvent) => {
        e.preventDefault();
        try {
            const confirm = await (window as anyObject).s_confirm('Logout');
            if (confirm) {
    ...
    return (
        <li className="icon_link_li app_drawar_link_li">
            ...
            {isDrawerOpen && (
                <>
                    <div className="apps_list_drawer" ref={drawerRef}>
                        <div className="content">
                            <div className="top">
                                <div className="profile">
                                    <div className="img">
                                        <img src="/assets/dashboard/images/avatar.png" alt="avatar" />
                                    </div>
                                ...
                                <div className="action">
                                    <ul>
                                        ...
                                        <li>
                                            <Link onClick={handleLogout} to="" className="sidebar-header">
                                                <span className="material-symbols-outlined fill">
                                                    power_settings_new
                                                </span>
                                            </Link>
                            ...
                            <div className="menu_list custom_scroll">
                                {(searchTerm ? filteredMenu : menuArray).map((group, groupIdx) => (
                                    <div className="menu_apart" key={groupIdx}>
                                        <h4>{group.group}</h4>
                                        <ul>
                                            {group.links.map((link, linkIdx) => (
                                                <li key={linkIdx} onClick={handleMenuItemClick}>
                                                    <a href={link.link}>
                                                        <div className={`icon bg_color_${linkIdx + 1}`}>
                                                            <span className="material-symbols-outlined fill">
                                                                {link.icon}
                                                            </span>
                                                        </div>
