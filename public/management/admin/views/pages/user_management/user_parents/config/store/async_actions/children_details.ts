...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(
        `${end_point}/user-students/children-details/${param.id}`,
    );

    dispatch(storeSlice.actions.set_is_loading(false));
    dispatch(storeSlice.actions.set_children(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const children_details = createAsyncThunk<
    ReturnType,
    PayloadType,
    ThunkArgument
>(`${store_prefix}/children-details`, fetch_api);
