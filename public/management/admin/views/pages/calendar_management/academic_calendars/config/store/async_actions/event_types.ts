...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(
        `${end_point}/academic-calendar-event-types/event-types`,
    );

    dispatch(storeSlice.actions.set_is_loading(false));
    dispatch(storeSlice.actions.set_types(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const event_types = createAsyncThunk<
    ReturnType,
    PayloadType,
    ThunkArgument
>(`${store_prefix}/event-types`, fetch_api);
