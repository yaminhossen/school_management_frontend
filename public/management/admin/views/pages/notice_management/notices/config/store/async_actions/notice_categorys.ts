...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(
        `${end_point}/${api_prefix}/notice-categorys`,
    );

    dispatch(storeSlice.actions.set_is_loading(false));
    dispatch(storeSlice.actions.set_categorys(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const notice_categorys = createAsyncThunk<
    ReturnType,
    PayloadType,
    ThunkArgument
>(`${store_prefix}/notice-categorys`, fetch_api);
