...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    // );
    const response = await axios.get(
        `${end_point}/account-fees-collection-details/student-payment-history/${param.id}`,
    );

    dispatch(storeSlice.actions.set_is_loading(false));
    dispatch(storeSlice.actions.set_payment(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const payment_history = createAsyncThunk<
    ReturnType,
    PayloadType,
    ThunkArgument
>(`${store_prefix}/payment-history`, fetch_api);
