...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    let url = `${api_host}${end_point}/${api_prefix}/class-details/${param.id}?secId=${param.secId}`; // Construct the URL with classId
    // if (param.id) {
    //     url += `/${param.id}`;
    // }
    ...
    dispatch(storeSlice.actions.set_id(param.id));
...
export const class_details1 = createAsyncThunk<
    ReturnType,
    PayloadType,
    ThunkArgument
>(`${store_prefix}/class-details1`, fetch_api);
