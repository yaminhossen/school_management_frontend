...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    const response = await axios.get(`${end_point}/${api_prefix}/pre-info`);

    dispatch(storeSlice.actions.set_is_loading(false));
    dispatch(storeSlice.actions.set_preInfo(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const preInfo = createAsyncThunk<ReturnType, PayloadType, ThunkArgument>(
    `${store_prefix}/preInfo`,
    fetch_api,
);
