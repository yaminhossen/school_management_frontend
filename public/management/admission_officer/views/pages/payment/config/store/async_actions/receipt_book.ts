...
const fetch_api = async (param: anyObject, thunkAPI) => {
    ...
    // );
    const response = await axios.get(`${end_point}/account-logs/receipt-book`);

    dispatch(storeSlice.actions.set_is_loading(false));
    // dispatch(storeSlice.actions.set_item(response.data.data));
    dispatch(storeSlice.actions.set_receipt(response.data.data));

    return response.data;
    // thunkAPI.dispatch(storeSlice.actions.my_action())
};

export const receipt_books = createAsyncThunk<
    ReturnType,
    PayloadType,
    ThunkArgument
>(`${store_prefix}/receipt_books`, fetch_api);
