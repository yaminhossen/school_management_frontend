...
window.s_confirm = async (
    ...
    const options = {
        ...
        confirmButtonText,
        customClass: {
            input: 'swal_input_light', // 👈 custom input class
        },
    };

    // ✨ If an expectedInput is passed, add input-related options
    if (expectedInput) {
        Object.assign(options, {
            input: 'text',
            inputPlaceholder: `Type: ${expectedInput}`,
            inputValidator: (value) => {
                if (value !== expectedInput) {
                    return `You must type exactly: "${expectedInput}"`;
                }
                return null;
            },
        });
    ...
    return (
        result.isConfirmed && (!expectedInput || result.value === expectedInput)
    );
};
...
window.s_confirm2 = async (
    title = 'Are you sure?',
    confirmButtonText = 'Yes, do it!',
    expectedInput = false // now this is a flag (not a specific string)
) => {
    const options ={
        title,
        showCancelButton: true,
        ...
        confirmButtonText,
        customClass: {
            input: 'swal_input_light',
        },
    };

    if (expectedInput) {
        Object.assign(options, {
            input: 'password', // use 'password' input type
    ...
    // Return the password if confirmed, otherwise null
