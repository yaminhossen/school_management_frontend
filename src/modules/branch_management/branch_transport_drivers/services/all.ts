...
async function all(
    ...
    if (search_key) {
        query.where = {
            ...
            [Op.or]: [
                ...
                { driver_number: { [Op.like]: `%${search_key}%` } },
                { assistant_number_1: { [Op.like]: `%${search_key}%` } },
                { assistant_number_2: { [Op.like]: `%${search_key}%` } },
                { present_address: { [Op.like]: `%${search_key}%` } },
                { licence_number: { [Op.like]: `%${search_key}%` } },
                { permanent_address: { [Op.like]: `%${search_key}%` } },
            ],
        };
    }
    ...
    try {
        let data = await (fastify_instance as anyObject).paginate(
            req,
            models.BranchTransportDriversModel,
            paginate,
            query,
        );
