...
async function task_assign_updated(
    ...
    console.log('form body form task assign page', body.id);
    ...
    try {
        ...
        let record = await models.TasksModel.findOne({
            where: {
                id: body.id,
            },
        });
        ...
        if (record) {
            // Delete the record
            (await record.update(inputs)).save();
        } else {
        ...
        // }
        return response(204, 'data created', data);
    } catch (error: any) {
...
export default task_assign_updated;
