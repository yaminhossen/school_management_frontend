...
import student_approved from './services/student_approved';
import student_pending from './services/student_pending';
import student_rejected from './services/student_rejected';
...
export default function (fastify: FastifyInstance) {
    return {
        ...
        teachers_approved: async function (
            req: FastifyRequest,
            res: FastifyReply,
        ) {
            let data = await teachers_approved(fastify, req);
            res.code(data.status).send(data);
        },

        student_approved: async function (
            req: FastifyRequest,
            res: FastifyReply,
        ) {
            let data = await student_approved(fastify, req);
            res.code(data.status).send(data);
        },

        student_pending: async function (
            req: FastifyRequest,
            res: FastifyReply,
        ) {
            let data = await student_pending(fastify, req);
            res.code(data.status).send(data);
        },

        student_rejected: async function (
            req: FastifyRequest,
            res: FastifyReply,
        ) {
            let data = await student_rejected(fastify, req);
            res.code(data.status).send(data);
        },

        teachers_rejected: async function (
            req: FastifyRequest,
            res: FastifyReply,
        ) {
            let data = await teachers_rejected(fastify, req);
            res.code(data.status).send(data);
        },

        teachers_pending: async function (
            req: FastifyRequest,
            res: FastifyReply,
        ) {
            let data = await teachers_pending(fastify, req);
            res.code(data.status).send(data);
        },
