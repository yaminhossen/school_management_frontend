...
async function validate(req: Request) {
    ...
    await body('branch_class_fee_id')
        .not()
        .isEmpty()
        .withMessage('the branch_class_fee_id field is required')
        .run(req);
...
async function update(
    ...
    let model = new models.BranchClassFeeDiscountsModel();
    ...
    let inputs: InferCreationAttributes<typeof model> = {
        ...
        branch_class_fee_id: body.branch_class_fee_id,
        title: body.title,
    ...
    try {
        let data = await models.BranchClassFeeDiscountsModel.findByPk(body.id);
        if (data) {
