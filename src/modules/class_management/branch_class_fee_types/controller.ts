...
import class_wise_fee_types from './services/class_wise_fee_types';
...
export default function (fastify: FastifyInstance) {
    return {
        ...
        class_wise_fee_types: async function (
            req: FastifyRequest,
            res: FastifyReply,
        ) {
            let data: responseObject = await class_wise_fee_types(fastify, req);
            res.code(data.status).send(data);
        },
